...
For container runs (Docker/Podman), datasource URL is loaded from `config/container-datasource.properties` and should remain:

```properties
spring.datasource.url=jdbc:h2:file:/app/data/dbconnector
```

Container data is persisted to host path `${HOME}/dbconnector/data` via compose bind mount.
